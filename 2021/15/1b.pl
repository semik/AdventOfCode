...
my @best_path; # 954 je zatim nejmin co jsme nasli
my $best_risk = 945; # zkouseno v 15:45
...
sub walkMap {
  ...
  foreach my $point (sort { $map->{$a->[1]}->{$a->[0]} <=> $map->{$b->[1]}->{$b->[0]} }  @possible) {
    #print $map->{$point->[1]}->{$point->[0]}."\n";
    walkMap($level+1, $map, $risk+$map->{$point->[1]}->{$point->[0]}, @path, $point);
  };
